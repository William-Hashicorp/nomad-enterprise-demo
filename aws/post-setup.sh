...
NOMAD_USER_TOKEN=$(curl -s --header "Authorization: Bearer ${CONSUL_TOKEN}" "${LB_ADDRESS}:8500/v1/kv/nomad_user_token?raw")
NOMAD_TOKEN=$(curl -s --header "Authorization: Bearer ${CONSUL_TOKEN}" "${LB_ADDRESS}:8500/v1/kv/nomad_bootstrap_token?raw")


# Save token to file if file doesn't already exist
if [ ! -f $NOMAD_USER_TOKEN_FILENAME ]; then
    echo $NOMAD_USER_TOKEN > $NOMAD_USER_TOKEN_FILENAME
    ...
    if [ ${#NOMAD_TOKEN} -eq 36 ]; then
        ...
        echo -e "\nThe Nomad UI can be accessed at ${LB_ADDRESS}:4646/ui\nwith the bootstrap token: $(echo $NOMAD_TOKEN)"
