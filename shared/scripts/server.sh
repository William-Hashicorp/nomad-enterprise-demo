...
RETRY_JOIN=$3
NOMAD_BINARY=$4
DATACENTER=$5  # Added datacenter parameter
REGION=$6      # Added region parameter
...
sed -i "s/DATACENTER/$DATACENTER/g" $CONFIGDIR/nomad.hcl  # Added datacenter replacement
sed -i "s/REGION/$REGION/g" $CONFIGDIR/nomad.hcl          # Added region replacement

sudo cp $CONFIGDIR/nomad.hcl $NOMADCONFIGDIR       # add nomad config file here
sudo cp $CONFIGDIR/license.hclic $NOMADCONFIGDIR   # add Enterprise license here
